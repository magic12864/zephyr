...
def create_parser(arg_list):
    ...
    parser.add_argument("--version", "-v",
                        type=lambda x: int(x, 16), dest="version")
...
def file_check(input, output = None):
    ...
    if output.exists():
        if output.samefile(input):
            raise RuntimeError(f'Input file {input} '
                               f'should be different from'
...
def main():
    ...
    # check file
    output_file = file_check(Path(input_file), Path(output_file))
